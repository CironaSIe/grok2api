...
attemptLoop:
	...
	handleResponse:
		...
		// Classify 403 bodies before egress/retry. Terminal safety / bare permission-denied
		// return as-is; blocked accounts invalidate; Build chat 403 rotates via CLI probe.
		if response.StatusCode == http.StatusForbidden {
			...
			if isTerminalRequestForbidden(credential.Provider, lastFailure) {
				// Request-scoped / unknown-scope 403: restore body and return without rotation.
				...
				// Fall through to the common response path so the client receives the original 403.
				...
				continue
			} else if egressForbidden && !finalEgressForbidden && !buildCLIForbidden {
		...
		if isTerminalRequestForbidden(credential.Provider, lastFailure) {
		...
		} else if (isRetryableResponse(response, route.Provider) || buildCLIForbidden) && !finalEgressForbidden {
			retryAfter := parseRetryAfter(response.Header.Get("Retry-After"), time.Now().UTC())
			body, _ := readRetryableBody(response.Body)
			lastFailure = newHTTPUpstreamFailure(response.StatusCode, body, credential.ID, credential.Name)
			...
			freeBuildForbidden := !buildCLIForbidden && isUnclassifiedFreeBuildForbidden(response.StatusCode, credential, lease.Billing, lastFailure, buildForbiddenReauth)
		...
		} else if response.StatusCode == http.StatusForbidden && credential.Provider == accountdomain.ProviderBuild && !isTerminalRequestForbidden(credential.Provider, lastFailure) {
			// Safety net: never hand non-terminal Build 403 to transport as Result.
			retryAfter := parseRetryAfter(response.Header.Get("Retry-After"), time.Now().UTC())
			body, _ := readRetryableBody(response.Body)
			lastFailure = newHTTPUpstreamFailure(response.StatusCode, body, credential.ID, credential.Name)
			lastFailure.AccountScoped = true
	...
	if lastFailure != nil {
		logRoutingDecision(s.logger, "fail", input.RequestID, route.Provider, lastFailure.AccountID, len(failureAttempts.snapshot()), lastFailure.Code)
		record := auditBase
