...
func TestBuildCLIProfileCRUDAndRoutingAttach(t *testing.T) {
	...
	candidates, err := accounts.ListRoutingCandidates(ctx, accountdomain.ProviderBuild, 0, "", "")
	if err != nil {
		t.Fatal(err)
	}
	...
	webCandidates, err := accounts.ListRoutingCandidates(ctx, accountdomain.ProviderWeb, 0, "", "")
	if err != nil {
		t.Fatal(err)
	}
