...
export type BuildConversionResultDTO = {
  ...
  synced: number;
  syncFailed: number;
  failures?: BuildConversionFailureDTO[];
};
...
const decodeAccountTaskStreamPayload = createObjectDecoder<AccountTaskStreamPayload>("account task event", {
  ...
  synced: isOptional(isNumber), syncFailed: isOptional(isNumber), failures: isOptional(isArrayOf(hasShape({ accountId: isNumber, message: isString }))),
  completed: isOptional(isNumber), total: isOptional(isNumber),
...
function buildConversionResultFromRecord(result: Record<string, unknown>): BuildConversionResultDTO {
  return {
    created: num(result.created), linked: num(result.linked), skipped: num(result.skipped), failed: num(result.failed),
    synced: num(result.synced), syncFailed: num(result.syncFailed), failures: decodeBuildConversionFailures(result.failures),
  };
}
...
export function convertWebAccountsToBuild(input: BuildConversionInput, onProgress?: (value: AccountTaskProgressDTO) => void, signal?: AbortSignal): Promise<BuildConversionResultDTO> {
  ...
  if (payload.async !== false) {
    return runJSONAdminTask("/api/admin/v1/accounts/web/convert-to-build", payload, onProgress, signal).then(buildConversionResultFromRecord);
  }
