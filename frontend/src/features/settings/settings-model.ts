...
export const settingsSchema = z.object({
  ...
  routing: z.object({
    ...
    cli: z.object({
      enabled: z.boolean(),
      warmTargetTotal: z.number().int().min(0).max(100_000),
      warmLowWatermarkRatio: z.number().min(0).max(1),
      warmMaxUnprovenShare: z.number().min(0).max(1),
      warmMaxUnprovenAbs: z.number().int().min(0).max(100_000),
      maxRefreshInflight: z.number().int().min(0).max(1_000),
      maxConvertInflight: z.number().int().min(0).max(100),
      maxConvertPerMinute: z.number().int().min(0).max(10_000),
      autoFillUnproven: z.boolean(),
      autoFillNonFree: z.boolean(),
      convertOnRequest: z.boolean(),
      layerHardPartition: z.boolean(),
      selectReadyOrRefreshableOnly: z.boolean(),
      autoPioneerFromWeb: z.boolean(),
      maxPioneerPerTick: z.number().int().min(0).max(100),
      pioneerPreferTrusted: z.boolean(),
      warmTickInterval: durationSchema.refine((value) => durationSeconds(value) >= 1 && durationSeconds(value) <= 3_600),
      accessRefreshAdvance: durationSchema.refine((value) => durationSeconds(value) >= 0 && durationSeconds(value) <= 3_600),
    }),
...
export function toSettingsForm(config: SettingsConfigDTO): SettingsForm {
  return {
    ...
    routing: {
      ...
      cli: {
        enabled: config.routing.cli?.enabled ?? true,
        warmTargetTotal: config.routing.cli?.warmTargetTotal ?? 500,
        warmLowWatermarkRatio: config.routing.cli?.warmLowWatermarkRatio ?? 0.8,
        warmMaxUnprovenShare: config.routing.cli?.warmMaxUnprovenShare ?? 0.4,
        warmMaxUnprovenAbs: config.routing.cli?.warmMaxUnprovenAbs ?? 120,
        maxRefreshInflight: config.routing.cli?.maxRefreshInflight ?? 25,
        maxConvertInflight: config.routing.cli?.maxConvertInflight ?? 3,
        maxConvertPerMinute: config.routing.cli?.maxConvertPerMinute ?? 20,
        autoFillUnproven: config.routing.cli?.autoFillUnproven ?? true,
        autoFillNonFree: config.routing.cli?.autoFillNonFree ?? true,
        convertOnRequest: config.routing.cli?.convertOnRequest ?? false,
        layerHardPartition: config.routing.cli?.layerHardPartition ?? true,
        selectReadyOrRefreshableOnly: config.routing.cli?.selectReadyOrRefreshableOnly ?? true,
        autoPioneerFromWeb: config.routing.cli?.autoPioneerFromWeb ?? true,
        maxPioneerPerTick: config.routing.cli?.maxPioneerPerTick ?? 5,
        pioneerPreferTrusted: config.routing.cli?.pioneerPreferTrusted ?? true,
        warmTickInterval: parseDuration(config.routing.cli?.warmTickInterval || "15s"),
        accessRefreshAdvance: parseDuration(config.routing.cli?.accessRefreshAdvance || "3m"),
      }
    },
...
export function toSettingsDTO(config: SettingsForm): SettingsConfigDTO {
  return {
    ...
    routing: {
      ...
      segmentedSelector: config.routing.segmentedSelector,
      cli: {
        enabled: config.routing.cli.enabled,
        warmTargetTotal: config.routing.cli.warmTargetTotal,
        warmLowWatermarkRatio: config.routing.cli.warmLowWatermarkRatio,
        warmMaxUnprovenShare: config.routing.cli.warmMaxUnprovenShare,
        warmMaxUnprovenAbs: config.routing.cli.warmMaxUnprovenAbs,
        maxRefreshInflight: config.routing.cli.maxRefreshInflight,
        maxConvertInflight: config.routing.cli.maxConvertInflight,
        maxConvertPerMinute: config.routing.cli.maxConvertPerMinute,
        autoFillUnproven: config.routing.cli.autoFillUnproven,
        autoFillNonFree: config.routing.cli.autoFillNonFree,
        convertOnRequest: config.routing.cli.convertOnRequest,
        layerHardPartition: config.routing.cli.layerHardPartition,
        selectReadyOrRefreshableOnly: config.routing.cli.selectReadyOrRefreshableOnly,
        autoPioneerFromWeb: config.routing.cli.autoPioneerFromWeb,
        maxPioneerPerTick: config.routing.cli.maxPioneerPerTick,
        pioneerPreferTrusted: config.routing.cli.pioneerPreferTrusted,
        warmTickInterval: formatDuration(config.routing.cli.warmTickInterval),
        accessRefreshAdvance: formatDuration(config.routing.cli.accessRefreshAdvance),
      }
    },
